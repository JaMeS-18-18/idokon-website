...
(function () {
  const wrap = document.querySelector('.cs_lang');
  const btn  = document.querySelector('.cs_lang_btn');
  const menu = document.querySelector('.cs_lang_menu');
  const items = menu ? menu.querySelectorAll('li[role="option"]') : [];
  const codeEl = btn?.querySelector('.cs_lang_code');
  const imgEl  = btn?.querySelector('img');

  const MAP = {
    uz: { flag: 'uz', label: "O'zbekcha" },
    ru: { flag: 'ru', label: "Русский" },
    en: { flag: 'gb', label: "English" }
  };

  function applyLang(lang){
    const m = MAP[lang] || MAP.uz;
    if (imgEl){ imgEl.src = `https://flagcdn.com/w20/${m.flag}.png`; imgEl.alt = m.label; }
    if (codeEl){ codeEl.textContent = m.label; }
    document.documentElement.setAttribute('lang', lang);
    localStorage.setItem('lang', lang);
    // TODO: Agar i18n (translatsiya) mavjud bo'lsa, shu yerda switch qiling.
  }
  ...
  // Initial
  applyLang(localStorage.getItem('lang') || 'uz');

  // Demo: savatcha badge (o'zingizning logikaga ulang)
  const badge = document.getElementById('cart-count');
  if (badge) {
    let c = Number(localStorage.getItem('cartCount') || 0);
    badge.textContent = c;
    // window.dispatchEvent(new CustomEvent('add-to-cart')) bilan oshirish mumkin
    window.addEventListener('add-to-cart', ()=>{
      c++; localStorage.setItem('cartCount', c); badge.textContent = c;
    });
  }
})();
